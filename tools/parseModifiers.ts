...
class ModifiersVisitor extends BaseJavaCstVisitorWithDefaults {
  ...
  processModifier(modifierType: keyof ModifiersVisitor['modifiers'], list: any) {
    const modifierDefinition = list.children.variableInitializer;
    const name = modifierDefinition[0].children.expression[0].children;
    const literal = name.ternaryExpression[0].children.binaryExpression[0].children.unaryExpression[0].children.primary[0].children.primaryPrefix[0].children.literal[0].children.StringLiteral[0].image as string;
    const unquoted = literal.substring(1, literal.length - 1);

    let mType = modifierType;

    if (unquoted in aggregateTypeMapping) {
      mType = aggregateTypeMapping[unquoted as keyof typeof aggregateTypeMapping];
    }

    this.modifiers[mType].push(unquoted);
  }

  processModifiers(modifierType: keyof ModifiersVisitor['modifiers'], list: any) {
    list.children.variableInitializer.forEach((v: any) => {
      this.processModifier(modifierType, v.children.arrayInitializer[0].children.variableInitializerList[0]);
    });
  }

  variableDeclaratorList(ctx: any) {
    const name = ctx.variableDeclarator[0].children.variableDeclaratorId[0].children.Identifier[0].image;
    
    const modifierType = Object.keys(this.modifiers).find(m => name === `${m}Modifiers`) as keyof ModifiersVisitor['modifiers'] | undefined;

    if (modifierType) {
      const list = ctx.variableDeclarator[0].children.variableInitializer[0].children.arrayInitializer[0].children.variableInitializerList[0];
...
async function main() {
  ...
  Object.entries(visitor.modifiers).forEach(([type, values]) => {
    ...
    contents += `export const ${typeName}Modifiers = ${JSON.stringify(values)} as const;\n`;
    contents += `export type ${tc(typeName)}Modifier = typeof ${typeName}Modifiers[number];\n`;
  });
