...
type Dungeon = "Hobopolis" | "Dreadsylvania" | "The Slime Tube";

const Loot: Record<Dungeon, Item[]> = {
  ...
  "The Slime Tube": $items`slime-soaked brain, slime-soaked hypophysis, slime-soaked sweat gland, squirming Slime larva, caustic slime nodule, caustic slime nodule, hardened slime belt, hardened slime hat, hardened slime pants`,
};
...
export function distribute(
  ...
  const badLoot = lootList.find(
    (individualLoot) =>
      !Object.values(Loot).some((dungeonLoot) =>
        dungeonLoot.includes(individualLoot)
      )
  );
  if (badLoot) {
    throw new Error(
...
export function closeDungeon(dungeon: Dungeon): boolean {
  const action =
    dungeon === "Hobopolis"
      ? "floodsewer"
      : dungeon === "The Slime Tube"
      ? "sealtube"
      : "foldmap";
  visitUrl(`clan_basement.php?action=${action}&confirm=true`);
  const pageText = visitUrl("clan_basement.php");
  const imageToCheck =
    dungeon === "Hobopolis"
      ? "opengrate.gif"
      ...
      ? "slimehole.gif"
      : "dvmap.gif";
  return !pageText.includes(imageToCheck);
}
...
export function openDungeon(
  ...
  const imageToCheck =
    dungeon === "Hobopolis"
      ...
      : "dvmap.gif";
  const pageText = visitUrl("clan_basement.php");
  if (pageText.includes(imageToCheck)) return true;

  const cost = dungeon === "The Slime Tube" ? 250000 : 1000000;
  if (paymentPolicy === "All") {
    visitUrl(`clan_stash.php?action=contribute&howmuch=${cost}`, true);
  } else {
    ...
    const payDifference = cost - stashMeat;
    if (payDifference > 0) {
