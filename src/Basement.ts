...
export function distribute(
  ...
  const lootList = Array.isArray(loot)
    ? loot
    : loot instanceof Dungeon
    ? loot.loot
    : [loot];
  const badLoot = lootList.find(
    (individualLoot) =>
      !Object.values(Dungeons)
        .map((dungeon) => dungeon.loot)
        .some((dungeonLoot) => dungeonLoot.includes(individualLoot))
  );
  if (badLoot) {
    throw new Error(
...
export function closeDungeon(dungeon: Dungeon): boolean {
  visitUrl(`clan_basement.php?action=${dungeon.closeAction}&confirm=true`);
  const pageText = visitUrl("clan_basement.php");
...
export function openDungeon(
  ...
  if (paymentPolicy === "All") {
    visitUrl(
      `clan_stash.php?action=contribute&howmuch=${dungeon.openCost}`,
      true
    );
  } else {
